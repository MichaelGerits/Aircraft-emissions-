...
dfEU=pd.read_csv("Data\PositionData\FPA202003\FPA202003_1.csv")
EuroControlID=dfEU["ECTRL ID"]
EuControl=[]
...
for i in EuroControlID:
    Planetype=AircraftDictionary_Eurocontrol_and_Aircraft[i]
    if Planetype[0]=="A" or Planetype[0]== "B"  or Planetype[0]== "C" or Planetype[0]== "E" or Planetype[0]== "G": #makes sure that only supported planes type are checked by the function
        PlaneType=aircraft_dict[AircraftDictionary_Eurocontrol_and_Aircraft[i]] #defines the planetype that will be used 
        print(PlaneType) 
        progress=progress+1
        print(progress/length*100)
        m_A320=aircraft_dict_mass[PlaneType]     #in kg 
        ...
        df=extract_ECTRLIDSeq('Data/PositionData/March')[i]  #imports the data
